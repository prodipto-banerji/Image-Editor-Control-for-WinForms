...
namespace CraftSynth.ImageEditor
{
    ...
    {
        // Contains the list of Layers - Using List<T> instead of ArrayList for better type safety
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                typeName = info.GetString(
                    String.Format(CultureInfo.InvariantCulture,
                                  "{0}{1}",
                                  entryLayer, i)) ?? string.Empty;

                if (!string.IsNullOrEmpty(typeName))
                {
                    object? _layer = Assembly.GetExecutingAssembly().CreateInstance(typeName);
                    if (_layer is Layer layer)
                    {
                        layer.LoadFromStream(info, i);
                        layerList.Add(layer);
                    }
                }
            }
        ...
        /// <summary>
        /// Draw all objects contained in all visible layers using SkiaSharp canvas
        /// </summary>
        /// <param name="canvas">SKCanvas object to draw on</param>
        ...
        /// <summary>
        /// Draw all objects contained in all visible layers (legacy Graphics compatibility method)
        /// This method provides backward compatibility but should be replaced with Draw(SKCanvas)
        /// </summary>
        /// <param name="g">Graphics object - will be converted to SKCanvas internally</param>
        [Obsolete("Use Draw(SKCanvas canvas) instead for better performance and cross-platform compatibility")]
        public void Draw(System.Drawing.Graphics g)
        {
            // This is a compatibility method. In a real implementation, you might need to:
            // 1. Create an SKCanvas from the Graphics object using a surface
            // 2. Or refactor all calling code to use SKCanvas directly
            
            // For now, we'll throw a NotSupportedException to encourage migration
            throw new NotSupportedException("Legacy Graphics drawing is not supported in .NET 9 migration. Please use Draw(SKCanvas canvas) instead.");
        }
        ...
        public void SetActiveLayer(int p)
        {
            ...
            {
                // Make sure nothing is selected on the currently active layer before switching layers.
                //if (layerList[ActiveLayerIndex].Graphics != null)
                //  layerList[ActiveLayerIndex].Graphics.UnselectAll();
                //layerList[ActiveLayerIndex].IsActive = false;

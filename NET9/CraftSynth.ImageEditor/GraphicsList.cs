...
namespace CraftSynth.ImageEditor
{
    ...
    public class GraphicsList : IDisposable
    {
        ...
        /// <summary>
        /// Draw all the visible objects in the List using SkiaSharp canvas
        /// </summary>
        /// <param name="canvas">SKCanvas to draw on</param>
        /// <param name="clipBounds">Clipping bounds for visibility testing</param>
        public void DrawSkia(SKCanvas canvas, SKRect clipBounds)
        {
            ...
            for (int i = numberObjects - 1; i >= 0; i--)
            {
                ...
                var objBounds = o.GetBoundsSkia();
                if (objBounds.IntersectsWith(clipBounds))
                {
                    // Call the SkiaSharp-based draw method
                    o.DrawSkia(canvas);
                }

                if (o.Selected)
                    o.DrawTrackerSkia(canvas);
            }
        }

        /// <summary>
        /// Legacy Draw method for compatibility with System.Drawing.Graphics
        /// This method creates a temporary bitmap and canvas for SkiaSharp rendering
        /// </summary>
        /// <param name="g">System.Drawing.Graphics object</param>
        [Obsolete("Use DrawSkia method instead for better performance")]
        public void Draw(System.Drawing.Graphics g)
        {
            // For backward compatibility, we can create an SKCanvas from the Graphics object
            // This requires additional work and is not optimal
            var clipBounds = g.ClipBounds;
            var skClipBounds = new SKRect(clipBounds.Left, clipBounds.Top, clipBounds.Right, clipBounds.Bottom);
            
            // Create a temporary bitmap and canvas
            var info = new SKImageInfo((int)clipBounds.Width, (int)clipBounds.Height);
            using (var surface = SKSurface.Create(info))
            {
                var canvas = surface.Canvas;
                canvas.Clear(SKColors.Transparent);
                
                // Translate canvas to match the clipping bounds
                canvas.Translate(-clipBounds.Left, -clipBounds.Top);
                
                DrawSkia(canvas, skClipBounds);
                
                // Convert back to System.Drawing if needed
                // This is a placeholder - in real implementation you'd need to 
                // either eliminate this method or implement proper Graphics interop
            }
        }
        ...
        /// <summary>
        /// Select objects within the specified rectangle (SkiaSharp version)
        /// </summary>
        /// <param name="rectangle">SKRect selection rectangle</param>
        public void SelectInRectangleSkia(SKRect rectangle)
        {
            UnselectAll();

            foreach (DrawObject o in graphicsList)
            {
                if (o.IntersectsWithSkia(rectangle))
                    o.Selected = true;
            }
        }

        /// <summary>
        /// Legacy method for compatibility with System.Drawing.Rectangle
        /// </summary>
        /// <param name="rectangle">System.Drawing.Rectangle selection rectangle</param>
        [Obsolete("Use SelectInRectangleSkia method instead")]
        public void SelectInRectangle(System.Drawing.Rectangle rectangle)
        {
            var skRect = new SKRect(rectangle.Left, rectangle.Top, rectangle.Right, rectangle.Bottom);
            SelectInRectangleSkia(skRect);
        }
        ...
        public bool DeleteSelection()
        {
            ...
            for (int i = n - 1; i >= 0; i--)
            {
                ...
                {
                    // Dispose the object before removing it
                    ((DrawObject)graphicsList[i]).Dispose();
                    graphicsList.RemoveAt(i);
                    result = true;
                }
        ...
        public void DeleteLastAddedObject()
        {
            ...
            {
                // Dispose the object before removing it
                if (graphicsList[0] != null)
                {
                    ((DrawObject)graphicsList[0]).Dispose();
                }
        ...
        public void Replace(int index, DrawObject obj)
        {
            if (index >= 0 &&
                index < graphicsList.Count)
            {
                // Dispose the old object before replacing it
                if (graphicsList[index] != null)
                {
                    ((DrawObject)graphicsList[index]).Dispose();
                }
        ...
        #region Helper Methods for SkiaSharp Migration
        ...
        /// <summary>
        /// Convert System.Drawing.RectangleF to SKRect
        /// </summary>
        /// <param name="rectangle">System.Drawing.RectangleF to convert</param>
        /// <returns>Equivalent SKRect</returns>
        public static SKRect ToSKRect(System.Drawing.RectangleF rectangle)
        {
            return new SKRect(rectangle.Left, rectangle.Top, rectangle.Right, rectangle.Bottom);
        }

        /// <summary>
        /// Convert SKRect to System.Drawing.Rectangle
        /// </summary>
        /// <param name="skRect">SKRect to convert</param>
        /// <returns>Equivalent System.Drawing.Rectangle</returns>
        public static System.Drawing.Rectangle ToDrawingRectangle(SKRect skRect)
        {
        ...
        /// <summary>
        /// Convert SKPoint to System.Drawing.Point
        /// </summary>
        /// <param name="skPoint">SKPoint to convert</param>
        /// <returns>Equivalent System.Drawing.Point</returns>
        public static System.Drawing.Point ToDrawingPoint(SKPoint skPoint)
        {

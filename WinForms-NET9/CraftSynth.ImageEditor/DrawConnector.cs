...
{
    /// <summary>
    /// Connector graphic object - series of straight lines with caps (SkiaSharp)
    /// </summary>
    public class DrawConnector : DrawLine
    {
        private SKStrokeCap startCap = SKStrokeCap.Butt;
        private SKStrokeCap endCap = SKStrokeCap.Round;
        ...
        public SKStrokeCap StartCap { get => startCap; set => startCap = value; }
        public SKStrokeCap EndCap2 { get => endCap; set => endCap = value; }
        public bool StartIsAnchored { get => startIsAnchored; set => startIsAnchored = value; }
        public bool EndIsAnchored { get => endIsAnchored; set => endIsAnchored = value; }
        public int StartObjectId { get => startObjectId; set => startObjectId = value; }
        public int EndObjectId { get => endObjectId; set => endObjectId = value; }
        ...
        ~DrawConnector() { Dispose(false); }

        public DrawConnector(float x1, float y1, float x2, float y2, SKColor lineColor, float lineWidth, DrawingPens.PenType penType, SKStrokeCap endCap)
        {
            pointArray.Add(new SKPoint(x1, y1));
            pointArray.Add(new SKPoint(x2, y2));
            TipText = string.Format(CultureInfo.InvariantCulture, "Start @ {0}-{1}, End @ {2}, {3}", x1, y1, x2, y2);
            Color = lineColor;
            PenWidth = lineWidth;
            PenType = penType;
            EndCap = endCap;
        ...
        public override void SaveToStream(SerializationInfo info, int orderNumber, int objectIndex)
        {
            ...
            int i = 0; foreach (SKPoint p in pointArray)
            {
